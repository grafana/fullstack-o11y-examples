# ruby / k8s — Observable Bookstore

Kubernetes manifests for the Ruby stack (same app as [`../docker-compose`](../docker-compose)):
...
[Python k8s reference](../../python/k8s); images are tagged `bookstore-ruby/*` to
avoid colliding with the other language images.
...
1. **Build & push the images** (no in-cluster build):
   ...
   ```bash
   cd ../docker-compose
   docker build -t <registry>/bookstore-ruby/products:latest -f services/products/Dockerfile services
...
- Databases use `StatefulSet` + `volumeClaimTemplates`; the init scripts run only
  ...
  `/var/lib/postgresql` (no `PGDATA`) for the postgres:18 layout.
